...
_BROWSER_MODES = {
    ...
    "local_launch",
    "local_persistent",
    # CloakBrowser stealth modes (Webwright-Clockbrowser combine).
    # cloak_launch:     fresh stealth Chromium per run (in-process).
...
class LocalBrowserEnvironmentConfig(BaseModel):
    ...
    # --- CloakBrowser stealth options (browser_mode=cloak_launch | cloak_persistent) ---
    # Behavioral humanization (mouse Bezier curves, per-character typing, scroll).
    # Applied in-process by CloakBrowser's launch_*_async helpers.
    cloak_humanize: bool = True
    cloak_human_preset: str = "default"  # "default" | "careful"
    # Residential proxy URL, e.g. "http://user:pass@host:port" or "socks5://...".
    cloak_proxy: str | None = None
    # Auto-detect timezone/locale (and spoof WebRTC IP) from the proxy's exit IP.
    cloak_geoip: bool = False
    # Explicit IANA timezone / BCP-47 locale (win over geoip when set).
    cloak_timezone: str | None = None
    cloak_locale: str | None = None
    # Include CloakBrowser's default source-level fingerprint flags.
    cloak_stealth_args: bool = True
    # Optional Chrome extensions to load into the stealth browser.
    cloak_extension_paths: list[str] = Field(default_factory=list)
...
class LocalBrowserEnvironment:
    ...
    async def _prepare_async(self) -> None:
        ...
        # CloakBrowser stealth modes own their own Playwright instance, so we
        # delegate entirely and skip starting Webwright's own Playwright.
        if self.config.browser_mode in ("cloak_launch", "cloak_persistent"):
            await self._prepare_cloak_async()
    ...
    async def _prepare_cloak_async(self) -> None:
        """Launch a CloakBrowser stealth Chromium in-process (combine Path 1).

        CloakBrowser's ``launch_*_async`` helpers start their own Playwright
        instance internally and patch ``.close()`` to stop it, so we do NOT start
        Webwright's own Playwright here. ``self._playwright`` therefore stays
        ``None`` — generated step code drives the page via ``page`` / ``context``
        / ``browser`` (the ``playwright`` arg is only needed by the vanilla
        modes). Stealth fingerprints are compiled into the binary and activated
        by launch args; ``humanize`` adds behavioral mimicry in-process.
        """
        try:
            import cloakbrowser
        except ImportError as exc:  # pragma: no cover - depends on optional install
            raise RuntimeError(
                "browser_mode='%s' requires the 'cloakbrowser' package. "
                "Install it with: pip install cloakbrowser" % self.config.browser_mode
            ) from exc
        ...
        opts: dict[str, Any] = dict(
            ...
            proxy=self.config.cloak_proxy or None,
            geoip=self.config.cloak_geoip,
            stealth_args=self.config.cloak_stealth_args,
        )
        if self.config.cloak_timezone:
            opts["timezone"] = self.config.cloak_timezone
        if self.config.cloak_locale:
            opts["locale"] = self.config.cloak_locale
        if self.config.cloak_extension_paths:
            opts["extension_paths"] = list(self.config.cloak_extension_paths)
        if self.config.launch_args:
            opts["args"] = list(self.config.launch_args)

        if self.config.browser_mode == "cloak_persistent":
            self.config.user_data_dir.mkdir(parents=True, exist_ok=True)
            ...
            self._browser = await cloakbrowser.launch_async(**opts)
    ...
    async def _run_python_code(self, python_code: str) -> None:
        # Note: CloakBrowser stealth modes manage their own Playwright instance,
        # so self._playwright is intentionally None there. Only page+context are
        # required; the `playwright` arg passed to step code may be None.
    ...
    async def _close_async(self) -> None:
        ...
        try:
            if connected_over_cdp:
                ...
                # Close context first, then the browser. For CloakBrowser
                # launch mode the browser's patched close() stops its internal
                # Playwright instance; for persistent mode the context's close()
                # does. Closing both (when present) is safe for vanilla modes.
