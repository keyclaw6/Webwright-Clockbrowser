...
```bash
pip install -e .          # installs cloakbrowser + deps
# CloakBrowser's stealth Chromium binary auto-downloads on first launch.
```
...
- Browser mode is **stealth**: every run launches a fresh CloakBrowser stealth
  Chromium via `from cloakbrowser import launch_async; await launch_async(
  headless=False, humanize=True)`. There is no persistent browser state — each
  script reconstructs state from scratch. (CloakBrowser replaces stock
  Webwright's Firefox fallback: its ja3/ja4/akamai TLS fingerprint matches real
  Chrome, so the `ERR_HTTP2_PROTOCOL_ERROR` that forced Firefox no longer
  occurs, and you additionally get full bot-detection evasion.)
