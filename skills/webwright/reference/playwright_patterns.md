...
## Browser launch skeleton (stealth mode — Webwright-Clockbrowser)

This build launches **CloakBrowser stealth Chromium** instead of vanilla
Playwright. Stock Webwright fell back to Firefox because Playwright Chromium
hit `ERR_HTTP2_PROTOCOL_ERROR` on Akamai-protected sites (cars.com, etc.) due
to TLS/H2 fingerprinting. CloakBrowser's ja3/ja4/akamai fingerprint is
identical to real Chrome, so that error goes away and you get a real Chromium
engine *plus* full bot-detection evasion. `cloakbrowser` is already installed
and its binary auto-downloads on first launch — do NOT pip/apt-install anything.
...
from cloakbrowser import launch_async  # stealth Chromium, drop-in Playwright API
...
async def main():
    # launch_async manages its own Playwright instance — no async_playwright() wrapper.
    ...
    await browser.close()  # also stops CloakBrowser's internal Playwright
...
async def main():
        # launch_async owns its own Playwright instance — no async_playwright() wrapper.
        browser = await launch_async(headless=False, humanize=True)
        context = await browser.new_context(viewport={"width": 1280, "height": 1800})
        page = await context.new_page()
